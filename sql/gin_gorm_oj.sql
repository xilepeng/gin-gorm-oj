...
CREATE TABLE `problem_basic` (
                                 ...
                                 `max_runtime` int DEFAULT NULL COMMENT '最大的运行时间',
                                 `max_mem` int DEFAULT NULL COMMENT '最大的运行内存',
                                 `pass_num` int DEFAULT '0' COMMENT '通过的问题个数',
                                 `submit_num` int DEFAULT '0' COMMENT '提交次数',
                                 `created_at` datetime DEFAULT NULL,
                                 `updated_at` datetime DEFAULT NULL,
                                 `deleted_at` datetime DEFAULT NULL,
                                 PRIMARY KEY (`id`),
                                 KEY `关联测试用例表` (`identity`),
                                 CONSTRAINT `关联测试用例表` FOREIGN KEY (`identity`) REFERENCES `test_case` (`identity`)
...
CREATE TABLE `problem_category` (
                                    `id` int unsigned NOT NULL AUTO_INCREMENT,
                                    `problem_id` int unsigned DEFAULT NULL COMMENT '问题ID',
                                    `category_id` int DEFAULT NULL COMMENT '分类ID',
                                    `created_at` datetime DEFAULT NULL,
                                    `updated_at` datetime DEFAULT NULL,
                                    `deleted_at` datetime DEFAULT NULL,
                                    PRIMARY KEY (`id`),
                                    KEY `关联问题分类表` (`problem_id`),
                                    CONSTRAINT `关联问题分类表` FOREIGN KEY (`problem_id`) REFERENCES `problem_basic` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=25 DEFAULT CHARSET=utf8mb3;
...
CREATE TABLE `submit_basic` (
                                ...
                                `deleted_at` datetime DEFAULT NULL,
                                PRIMARY KEY (`id`),
                                KEY `关联问题基础表` (`problem_identity`),
                                KEY `关联用户基础表` (`user_identity`),
                                CONSTRAINT `关联用户基础表` FOREIGN KEY (`user_identity`) REFERENCES `user_basic` (`identity`),
                                CONSTRAINT `关联问题基础表` FOREIGN KEY (`problem_identity`) REFERENCES `user_basic` (`identity`)
...
CREATE TABLE `user_basic` (
                              ...
                              `pass_num` int DEFAULT '0' COMMENT '完成问题的个数',
                              `submit_num` int DEFAULT '0' COMMENT '总提交次数',
